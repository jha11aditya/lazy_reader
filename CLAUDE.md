...
- [ ] **Phase 5b: Play Store release prep (v1)**
  ...
  - [ ] Enable R8/minify with MediaPipe/Room keep rules; build `.aab` via `bundleRelease`.
  - [ ] Launcher icon (Lazy Santa design — assets in `design/icons/`, currently paused).
  - [ ] Hosted privacy policy URL (required for RECORD_AUDIO) + Play data-safety form ("no data collected").
